...
resource "azurerm_virtual_machine" "vmConfigurator" {    
    ...
    network_interface_ids   = [ azurerm_network_interface.myNic[index(azurerm_network_interface.myNic.*.name, "vmnic-${var.vm_configurator_config[count.index].name}")].id ]
    ...
    provisioner "file" {
        ...
        connection {
            ...
            host = "${azurerm_public_ip.myPublicIp[index(azurerm_public_ip.myPublicIp.*.name, "vmip-${var.vm_configurator_config[count.index].name}-PublicIP")].domain_name_label}.westeurope.cloudapp.azure.com"
        }
    }
